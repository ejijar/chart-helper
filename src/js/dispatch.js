  ...
  bucketItems.forEach(item => {
    ...
    el.innerHTML = `
      <div class="feed-item-header">
        ...
        <span class="feed-item-time">${item.processed ? '<span style="color:var(--success);font-size:10px;margin-right:4px;">✓</span>' : ''}${item.timestamp}</span>
...
(function initLightboxPinchZoom() {
  ...
  document.addEventListener('touchstart', (e) => {
    ...
    if (e.touches.length === 2) {
      ...
      originX = m.x - rect.left;
      originY = m.y - rect.top;
      lastMidX = m.x;
      lastMidY = m.y;
      ...
      isSingleDrag = true;
      singleStartX = e.touches[0].clientX - translateX;
      singleStartY = e.touches[0].clientY - translateY;
    }
  }, { passive: false });
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    if (isPinching && e.touches.length === 2) {
      ...
      scale = Math.min(4, Math.max(1, lastScale * (newDist / startDist)));
      const m = mid(e.touches);
      translateX = lastTranslateX + (m.x - lastMidX);
      translateY = lastTranslateY + (m.y - lastMidY);
      applyTransform(img);
    } else if (isSingleDrag && e.touches.length === 1) {
      e.preventDefault();
      translateX = e.touches[0].clientX - singleStartX;
      translateY = e.touches[0].clientY - singleStartY;
      applyTransform(img);
    }
  }, { passive: false });
