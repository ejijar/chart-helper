...
async function toggleBucketVoice() {
  ...
  if (!isCapturing) {
    ...
    if (useWhisper()) {
      // Whisper path — record then transcribe on stop
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          try {
            ...
            if (text) addBucketItem({ type: 'voice', url: null, transcript: text, engine: 'Whisper' });
          } catch(err) {
      ...
      // WebSpeech path — live transcription, no audio saved
      if (!hasWebSpeech()) {
      ...
      bucketVoiceLoop = makeWsLoop({
        ...
        onDone: () => {
          ...
          const text = bucketVoiceTranscript.trim();
          if (text) addBucketItem({ type: 'voice', url: null, transcript: text, engine: 'WebSpeech' });
          bucketVoiceTranscript = '';
        }
      });
    }
...
function getBucketBody(item) {
  ...
  if (item.type === 'voice') {
    // Transcribed voice — no audio blob, show transcript text only
    ...
    return `<div class="feed-voice-transcribed">
      <div class="feed-voice-header">
        ...
        <span class="feed-voice-engine">${engineLabel}</span>
      </div>
      <div class="feed-transcript">${item.transcript || '<em>No transcript</em>'}</div>
    </div>`;
  }
  if (item.type === 'photo') {
