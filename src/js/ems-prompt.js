...
- PHOTO VITALS TABLE RULE: If a photo contains a vitals table with multiple rows, treat each row as a fully atomic set. ALL values in a single row (BP, HR, RR, SpO2, etc.) belong together on one card and must never be split across cards or mixed with values from a different row or a different source. Map each row to a card by its timestamp (e.g. 7PM row → cardId 2, 7:15 row → cardId 3). Photo vitals take precedence over note-sourced vitals for the same card — overwrite the note value, do not blend or average. If a cell in the photo row is blank, leave that field empty on the card rather than pulling it from notes.
